...
@deprecated
def train_model(video_file: str = 'video.mp4', device='auto', num_epochs=15, skip_frames=10, save_interval=10, num_frames=10, original_size=(1920, 1080), target_size=(1280, 720)) -> SrCnn:
    ...
    for epoch in pbar:
        ...
        for i, frames in enumerate(dataset):
            ...
            if not i % skip_frames:
                prev_high_res_frame, low_res_frame, high_res_frame = [f.unsqueeze(0).to(device) for f in frames]
            else:
                low_res_frame, high_res_frame = [f.unsqueeze(0).to(device) for f in frames if f is not None]
            optimizer.zero_grad()
        ...
        if epoch % save_interval == 0:
            difference = get_bw_difference(model, prev_high_res_frame, low_res_frame, high_res_frame)
...
if __name__ == '__main__':
    train_model(num_epochs=10, video_file='videos/video.mp4', num_frames=None, skip_frames=10, target_size=(960, 540), save_interval=1)
